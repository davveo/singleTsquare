...
func UserRouter(router *gin.RouterGroup) {
	...
	userOuterRouter := router.Group("user").Use(middleware.JwtMiddleWare())
	{
		...
		userOuterRouter.GET("user_id", controller.UserGet)                 //获取用户信息
		userOuterRouter.PUT("user_id", controller.UserUpdate)              //修改用户信息
		userOuterRouter.POST("bind", controller.BindAccount)               //第三方登录绑定
		userOuterRouter.POST("verify_code", controller.VerifyCode)         //验证手机号+code
